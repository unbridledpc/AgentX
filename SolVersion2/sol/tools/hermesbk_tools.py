...
from sol.core.journal import JournalError
from sol.tools.base import Tool, ToolArgument, ToolExecutionError
from sol.tools.web import WebAccessDenied, fetch_text
...
class PatchApplyTool(Tool):
    ...
    args = (
        ...
        ToolArgument("backup", bool, "Keep rollback artifacts (always true in SolVersion2)", required=False, default=True),
        ...
        try:
            ...
            for rec in reversed(applied):
                ...
                if not backup_path or not target_path:
                    continue
                try:
                    # backup artifacts live under SolVersion2/data/rollback and may be on a different drive.

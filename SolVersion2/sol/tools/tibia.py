...
from sol.core.forum_extract import extract_forum_text_and_meta
...
from sol.tools.web_ingest_url import _fetch_html_checked, _parse_robots, _robots_allows, _robots_url
from sol.tools.web import WebAccessDenied
...
def _distill_sol_notes(*, domain: str, domain_key: str, thread_title: str | None, start_url: str, pages: list[dict[str, Any]]) -> str:
    title = (thread_title or "").strip() or "<unknown thread>"
    lines: list[str] = []
    lines.append(f"SOL NOTES (tibia forums) — {domain_key} ({domain})")
            ...
            pages.append(
                {
                    ...
                    "page_num": None,
                    "file": note_file,
                    "title": f"Sol Notes: {thread_title or scope.domain_key}",

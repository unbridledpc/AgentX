...
def _infer_app_root_from_paths(paths_d: dict[str, Any], *, fallback: Path) -> Path:
    candidates = [
        ...
        _as_str(paths_d.get("solweb_dir"), ""),
    ]
    for raw in candidates:
        ...
        for anchor in ("apps", "SolWeb", "SolVersion2"):
            if anchor in parts:
...
def load_config(path: str) -> SolConfig:
    cfg_path = Path(path).expanduser()
    app_root_env = (os.environ.get("SOL_APP_ROOT") or "").strip()
    app_root = Path(app_root_env).expanduser().resolve() if app_root_env else None
    runtime_root_env = (os.environ.get("SOL_RUNTIME_ROOT") or "").strip()
    runtime_root = Path(runtime_root_env).expanduser().resolve() if runtime_root_env else None
    working_dir_env = (os.environ.get("SOL_WORKING_DIR") or "").strip()
    ...
    if app_root is None:
        ...
        if raw_path.name == "sol.toml" and raw_path.parent.name == "config":
    ...
    plugins_dir_raw = Path(_as_str(paths_d.get("plugins_dir"), "SolVersion2/plugins"))
    skills_dir_raw = Path(_as_str(paths_d.get("skills_dir"), "SolVersion2/skills"))
    ...
    features_dir_raw = Path(_as_str(paths_d.get("features_dir"), "SolVersion2/Server/data/features"))
    api_dir_raw = Path(_as_str(paths_d.get("api_dir"), "apps/api"))
    web_dir_raw = Path(_as_str(paths_d.get("web_dir"), "SolWeb"))
    web_dist_dir_raw = Path(_as_str(paths_d.get("web_dist_dir"), "SolWeb/dist"))
    ...
    solweb_dir_raw_s = _as_str(paths_d.get("solweb_dir"), "").strip()
    desktop_dir_raw_s = _as_str(paths_d.get("desktop_dir"), "").strip()
    solweb_dir: Path | None = None
    desktop_dir: Path | None = None
    if solweb_dir_raw_s:
        solweb_dir_raw = Path(solweb_dir_raw_s)
        solweb_dir = _resolve_app_path(solweb_dir_raw).resolve()
    ...
    audit_path_raw = Path(_as_str(audit_d.get("log_path"), str(audit_dir / "sol_audit.jsonl")))
    ...
    web_cfg = WebConfig(
        ...
        user_agent=_as_str(web_d.get("user_agent"), "SolVersion2/0.1"),
    ...
    voice_cfg = VoiceConfig(
        ...
        wake_word=_as_str(voice_d.get("wake_word"), "sol"),

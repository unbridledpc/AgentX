...
from sol.cli.run import compose_run_prompt, interactive_run, read_stdin_text, read_task_file, run_task_via_api
from sol.install.bootstrap import read_bootstrap_app_root
from sol.install import (
...
from sol.install.wizard import app_root_sanity_error, build_install_config, fatal_bundle_validation_errors, prompt_install_config, render_setup_summary, validate_install_config
...
def _print_doctor_report(result: dict) -> None:
    section("Doctor", subtitle="Inspecting NexAI install, launcher, runtime, and service readiness.")
    key_value_table(
        "Resolved Paths",
        [
    ...
    if result.get("problems"):
        failure_panel("Doctor Found Problems", "NexAI is not fully ready yet.", guidance=[str(item) for item in result["problems"]], log_path=str(result["paths"]["install_log_path"]))
    else:
        next_steps_panel(
            "Doctor Summary",
            ["Managed runtime looks healthy.", "Launcher and config are ready.", "You can start NexAI with `nexai start`."],
        )
...
def _print_start_result(result: dict) -> None:
    section("Startup", subtitle="Bringing NexAI services online.")
    ...
    if failed:
        failure_panel(
            "Startup Failed",
            "One or more NexAI services could not start.",
            guidance=[
                "Run `nexai doctor` to inspect the install.",
        ...
        next_steps_panel(
            "Next Steps",
            ["Run `nexai status` to confirm health.", "Open the web UI if it is enabled.", "Use `nexai doctor` if something looks off."],
        )
...
def _print_service_status(result: dict[str, object]) -> None:
    ...
    section("Service Status", subtitle="Operational summary for NexAI managed services.")
    key_value_table(
        "Install",
        [
            ("Service mode", str(result.get("service_mode", "unknown"))),
...
def _print_logs_header(config, service: str, tail: int, result: dict[str, object]) -> None:
    section("Logs", subtitle="Recent NexAI service output.")
    source = ""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help="Path to NexAI runtime TOML config (default: config/sol.toml)",
    )
    p.add_argument(
        "--install-config",
        default=str(default_install_config_path()),
        help="Path to NexAI install metadata JSON",
    )

    sub = p.add_subparsers(dest="cmd", required=True)

    run = sub.add_parser("run", help="Run a task through the local NexAI API, or start the interactive CLI with no task")
    run.add_argument("--file", default="", help="Read plain UTF-8 text input from a file")
    run.add_argument("task", nargs=argparse.REMAINDER, help="Task to send to the NexAI API")
    ...
    selfcheck.add_argument("--exercise-cli-tool-wrappers", action="store_true", help="Also exercise `python -m sol tool ...` wrappers (slower)")
    ...
    setup = sub.add_parser("setup", help="Create or update a product-style NexAI installation")
    ...
    sub.add_parser("start", help="Start installed NexAI services")
    ...
    logs = sub.add_parser("logs", help="Show NexAI service logs")
    logs.add_argument("service", choices=("api", "web"))
    logs.add_argument("--tail", type=int, default=100, help="Number of lines to show")
    service = sub.add_parser("service", help="Manage NexAI systemd-user services")
    ...
    service_sub.add_parser("enable", help="Enable NexAI systemd-user services")
    service_sub.add_parser("disable", help="Disable NexAI systemd-user services")
    ...
    config_sub.add_parser("show", help="Show install metadata and generated sol.toml path")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "setup":
        ...
        if args.app_root:
            ...
            if sanity_error:
                failure_panel(
                    "Invalid App Bundle",
                    "NexAI could not determine a valid app bundle root.",
        ...
        if args.non_interactive:
            runtime_root = Path(args.runtime_root).expanduser().resolve() if args.runtime_root else (Path.home() / ".local" / "share" / "sol")
            working_dir = Path(args.working_dir).expanduser().resolve() if args.working_dir else (Path.home() / "sol-work")
            web_enabled = None
            if args.web_enabled:
        ...
        try:
            ...
            failure_panel(
                ...
                guidance=[
                    ...
                    "Fix the reported issue and run `nexai setup` again.",
                ],
        ...
        if not bool(runtime_info.get("ready")):
            failure_panel(
                ...
                guidance=[
                    "Run `nexai runtime inspect` for a detailed runtime report.",
    ...
    if args.cmd in {"start", "stop", "restart", "status", "uninstall", "doctor", "health", "paths", "runtime", "service", "logs"} or (args.cmd == "config" and args.config_cmd == "show") or args.cmd == "internal":
        ...
        if args.cmd == "doctor":
            if getattr(args, "fix", False):
                result = run_with_status("Running NexAI doctor with safe fixes", apply_safe_doctor_fixes, install, install_config_path=install_cfg_path)
                ...
                if final_status == "DEGRADED" and fixes_applied:
                    return 1
                return 2
            result = run_with_status("Running NexAI diagnostics", run_doctor, install)
        ...
        if args.cmd == "service":
            ...
            try:
                ...
                if args.service_cmd == "enable":
                    ...
                    ui_success("NexAI systemd-user units enabled.")
                    _print_service_toggle_summary(
                        "Auto-start Enabled",
                        states,
                        note="NexAI will start automatically on login when the user systemd session is active.",
                    )
                    return 0
                if args.service_cmd == "disable":
                    states = run_with_status("Disabling systemd-user units", disable_systemd_units, install)
                    ui_success("NexAI systemd-user units disabled.")
                    _print_service_toggle_summary(
                        "Auto-start Disabled",
                        states,
                        note="NexAI will no longer start automatically on login until the units are enabled again.",
                    )
                    return 0
                if args.service_cmd == "status":
                    result = run_with_status("Inspecting NexAI service status", status_installation, install)
    ...
    if args.config == "config/sol.toml" and install_cfg_path.exists():

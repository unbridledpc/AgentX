...
from sol.install.store import default_install_config_path
from sol.install.ui import (
    BRAND_NAME,
    BRAND_SUBTITLE,
    SummaryItem,
    bullet_list,
...
def app_root_sanity_error(app_root: Path) -> str | None:
    ...
    if any(marker in normalized for marker in _INVALID_APP_ROOT_MARKERS):
        return (
            f"Invalid NexAI app bundle root detected: {app_root}\n"
            "This points inside a Python environment, not a NexAI bundle.\n"
            "Re-run install-sol.sh or repair the bootstrap launcher."
        )
    return None
...
def fatal_bundle_validation_errors(errors: tuple[str, ...] | list[str]) -> list[str]:
    return [
        ...
        or "Invalid NexAI app bundle root detected" in str(item)
    ]
...
def _normalize_setup_path_input(
    ...
    *,
    ...
    if getattr(platform_info, "is_linux", False) and "\\" in text:
        raise ValueError(
            f"{label} must use Linux-style paths such as `/home/user/sol` or `/srv/sol`. "
...
def profile_description(profile: InstallProfile) -> str:
    return {
        InstallProfile.CLI: "Run NexAI from the terminal only (no web UI).",
        InstallProfile.STANDARD: "Recommended: NexAI CLI + API + Web UI.",
        InstallProfile.SERVER: "Run NexAI as a background service with the API only (no UI).",
        InstallProfile.DEVELOPER: "Full NexAI development setup with CLI, API, web UI, and broader local access.",
    }[profile]
...
def _path_writable(path: Path) -> bool:
    ...
    try:
        ...
        probe = probe_dir / ".sol-write-test"
...
def validate_install_config(config: InstallConfig) -> ValidationResult:
    ...
    if not (config.app_root / "SolVersion2" / "sol").exists():
        errors.append(f"App root does not look like a NexAI bundle: {config.app_root}")
    ...
    if config.web.enabled:
        web_dist = config.app_root / "SolWeb" / "dist" / "index.html"
    ...
    if provider == "openai" and not os.environ.get("SOL_OPENAI_API_KEY"):
        warnings.append("OpenAI provider selected but SOL_OPENAI_API_KEY is not set in the current environment.")
    ...
    if config.service_mode == ServiceMode.SYSTEMD_USER and not platform_info.systemd_user_available:
        warnings.append("systemd user services are not active in this environment. NexAI can still run with direct lifecycle commands.")
    if not (config.app_root / "SolVersion2" / "pyproject.toml").exists():
        warnings.append("SolVersion2 packaging metadata is missing from the app bundle. Automatic dependency provisioning may be unavailable.")
...
def _run_preflight(*, app_root: Path, install_cfg_path: Path) -> tuple[list[PreflightCheck], list[str]]:
    ...
    bootstrap_root = Path.home() / ".local" / "share" / "sol" / "bootstrap"
    runtime_root = Path.home() / ".local" / "share" / "sol"

    if sys.version_info >= (3, 10):
        checks.append(PreflightCheck("pass", "Python version", f"Using Python {sys.version.split()[0]}"))
    else:
        failures.append("Install or use Python 3.10 or newer, then run `sol setup` again.")
        checks.append(PreflightCheck("fail", "Python version", f"Python {sys.version.split()[0]} is too old for NexAI"))
    ...
    existing_nexai = shutil.which("nexai")
    expected_nexai = launcher_dir / "nexai"
    if existing_nexai and Path(existing_nexai).resolve(strict=False) != expected_nexai.resolve(strict=False):
        checks.append(PreflightCheck("warn", "Command shadowing", f"`nexai` on PATH resolves to {existing_nexai}. NexAI prefers {expected_nexai}."))
    elif existing_nexai:
        checks.append(PreflightCheck("pass", "Command shadowing", f"`nexai` resolves to {existing_nexai}"))
    else:
        checks.append(PreflightCheck("warn", "Command shadowing", "No `nexai` launcher is currently on PATH. Setup will still complete."))
    existing_sol = shutil.which("sol")
    if existing_sol:
        checks.append(PreflightCheck("warn", "Legacy sol command", f"`sol` may resolve to a system game or another command: {existing_sol}. Use `nexai` as the supported command."))

    return checks, failures


def _render_preflight(*, app_root: Path, install_cfg_path: Path) -> None:
    section("System Check", subtitle="Validating your environment before NexAI asks for install choices.")
    ...
    if failures:
        ...
        failure_panel(
            "Preflight Failed",
            "NexAI setup cannot continue until these issues are fixed.",
            guidance=failures,
        )
        raise RuntimeError("Preflight checks failed.")
    spacer()
    ui_success("System check passed. NexAI is ready to configure.")
...
def _profile_prompt() -> PromptSpec:
    return PromptSpec(
        ...
        prompt="Choose how you want to run NexAI",
        default="standard",
        help_text=(
            "What this controls:\n"
            "- Picks the main way NexAI will be used.\n"
            "Why it matters:\n"
            ...
            "Profiles:\n"
            "- cli: Run NexAI from the terminal only (no web UI).\n"
            "- standard: Recommended: NexAI CLI + API + Web UI.\n"
            "- server: Run NexAI as a background service (no UI).\n"
            "- developer: Full NexAI development setup with all tools and local access."
        ),
    )
...
def _runtime_prompt() -> PromptSpec:
    return PromptSpec(
        ...
        prompt="Where should NexAI store its runtime data",
        default=str(Path.home() / ".local" / "share" / "sol"),
        help_text=(
            "What is stored here:\n"
            "- logs, memory databases, jobs, runtime extensions, caches, and audit data.\n"
            "Why it matters:\n"
            "- This is NexAI's writable state directory.\n"
            "Safe default:\n"
...
def _working_prompt() -> PromptSpec:
    return PromptSpec(
        ...
        prompt="Where should NexAI work on your files by default",
        default=str(Path.home() / "sol-work"),
        help_text=(
            "What this is:\n"
            "- The default folder where NexAI will operate on files and projects.\n"
            "Why it matters:\n"
...
def _provider_prompt() -> PromptSpec:
    return PromptSpec(
        ...
        help_text=(
            "What this controls:\n"
            "- Which model backend NexAI uses for assistant responses and planning.\n"
            "Why it matters:\n"
            "- It changes whether NexAI runs local models, remote API models, or no real model at all.\n"
            "Options:\n"
...
def _service_prompt() -> PromptSpec:
    return PromptSpec(
        ...
        prompt="Install NexAI as a background service",
        default="none",
        help_text=(
            "What this controls:\n"
            "- Whether NexAI writes a user service so it can be started through systemd.\n"
            "Why it matters:\n"
            "- Useful if you want NexAI to run in the background more like a system app.\n"
            "Options:\n"
            "- none: Safe default. Start NexAI manually with `nexai start`.\n"
...
def _ollama_endpoint_prompt(*, platform_info, default_url: str) -> PromptSpec:
    ...
    return PromptSpec(
        ...
        help_text=(
            "What this is:\n"
            "- The base URL NexAI will use to talk to Ollama.\n"
            "Why it matters:\n"
            "- NexAI cannot discover models or run Ollama chats unless this endpoint is reachable.\n"
            "Examples:\n"
            f"- {DEFAULT_OLLAMA_BASE_URL}\n"
            "- http://192.168.1.50:11434\n"
            "- http://host.docker.internal:11434\n"
            "When to change it:\n"
            "- Change it if Ollama runs on another machine, in another container, or on Windows while NexAI runs inside WSL."
            + extra
        ),
    )
...
def _ollama_host_prompt(default_host: str) -> PromptSpec:
    return PromptSpec(
        ...
        help_text=(
            ...
            "Why it matters:\n"
            "- NexAI builds the full Ollama endpoint from this host and the port.\n"
            "Normal examples:\n"
            "- 127.0.0.1\n"
            "- localhost\n"
            "- 192.168.68.50\n"
            "Power-user option:\n"
            "- You can also paste a full URL like http://192.168.68.50:11434 and NexAI will use it directly."
        ),
    )
...
def _ollama_port_prompt(default_port: str) -> PromptSpec:
    return PromptSpec(
        ...
        help_text=(
            ...
            "Why it matters:\n"
            "- NexAI combines this port with the Ollama host to form the endpoint.\n"
            "Safe default:\n"
...
def render_setup_summary(
    ...
    *,
    ...
    lines = [
        ...
        f"- Start NexAI: {launcher_path} start" if launcher_path else f"- Start NexAI: nexai --install-config {default_install_config_path()} start",
        f"- Check status: {launcher_path} status" if launcher_path else f"- Check status: nexai --install-config {default_install_config_path()} status",
    ]
...
def _profile_mode_prompt() -> PromptSpec:
    return PromptSpec(
        title="Local Profile",
        prompt="Set a local NexAI profile or skip it",
        default="skip",
        help_text=(
            "What this controls:\n"
            "- Whether NexAI stores a simple local profile for display and internal identity.\n"
            "Why it matters:\n"
            "- NexAI uses this for local labels, audit metadata, and runtime defaults.\n"
            "Options:\n"
            "- set: Create a local profile with a display name and optional profile ID.\n"
            "- skip: Do not create one. NexAI will fall back to your OS username automatically.\n"
            "Safe default:\n"
            "- `skip` is fine if you do not care about naming NexAI locally yet."
        ),
    )
...
def _display_name_prompt(default_display_name: str) -> PromptSpec:
    return PromptSpec(
        title="Display Name",
        prompt="Choose a local display name for NexAI to use",
        default=default_display_name,
        help_text=(
            "What this is:\n"
            "- A friendly local name NexAI will use in labels and local metadata.\n"
            "Why it matters:\n"
            "- It makes logs and local runtime context easier to recognize.\n"
            "When to change it:\n"
            "- Change it if you want NexAI to use your preferred name instead of the OS-derived default.\n"
            "Example:\n"
...
def prompt_install_config(*, app_root: Path) -> tuple[InstallConfig, Path, LocalProfileSelection | None]:
    ...
    if sanity_error:
        failure_panel(
            "Invalid App Bundle",
            "NexAI setup cannot continue with the detected app root.",
            guidance=sanity_error.splitlines(),
        )
        raise RuntimeError("Invalid NexAI bundle root. Re-run install-sol.sh or repair the bootstrap launcher.")
    show_logo()
    info_items = [
    ...
    while True:
        ...
        if model_provider == "ollama":
            stage(6, total_steps, "Ollama Endpoint", subtitle="Tell NexAI where Ollama is running")
        ...
        api_host = _ask(
            PromptSpec(
                ...
                help_text=(
                    "What this is:\n"
                    "- The network address the NexAI API listens on.\n"
                    "Why it matters:\n"
        ...
        api_port = int(
            _ask(
                PromptSpec(
                    title="API Port",
                    prompt="Which port should the NexAI API use",
                    default="8420",
                    help_text=(
                        "What this is:\n"
                        "- The port used by NexAI's API.\n"
                        "Why it matters:\n"
        ...
        web_enabled = _ask(
            PromptSpec(
                ...
                help_text=(
                    "What this controls:\n"
                    "- Whether NexAI's browser-based UI is enabled.\n"
                    "Why it matters:\n"
                    "- The web UI gives you a graphical way to use NexAI.\n"
                    "When to change it:\n"
        ...
        web_host = _ask(
            PromptSpec(
                ...
                help_text=(
                    "What this is:\n"
                    "- The network address used by the NexAI web UI server.\n"
                    "Why it matters:\n"
        ...
        web_port = int(
            _ask(
                ...
                help_text=(
                    "What this is:\n"
                    "- The port used by the NexAI browser UI.\n"
                    "Why it matters:\n"
        ...
        stage(10, total_steps, "Validation and Provisioning Readiness", subtitle="Review warnings before NexAI provisions its runtime")
        ...
        if fatal_errors:
            failure_panel(
                "Invalid App Bundle",
                "NexAI setup cannot continue because the app bundle path is invalid.",
                guidance=fatal_errors + ["Fix the bundle path and re-run `sol setup`."],
            )
            raise RuntimeError("Invalid NexAI bundle root. Re-run install-sol.sh or repair the bootstrap launcher.")
        if validation.ok:
            ...
            next_steps_panel(
                ...
                [
                    ...
                    "Run health checks before starting NexAI services.",
                ],

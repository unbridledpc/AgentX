...
def _audit_path(cfg: Any | None) -> Path:
    override = (os.environ.get("SOL_UNSAFE_AUDIT_PATH") or "").strip()
    if override:
        return Path(override)
    if cfg is not None and getattr(cfg, "paths", None) is not None:
        base = Path(getattr(cfg.paths, "audit_dir", cfg.paths.data_dir))
    else:
        runtime_root = Path(os.environ.get("SOL_RUNTIME_ROOT") or Path.home() / ".local" / "share" / "sol")

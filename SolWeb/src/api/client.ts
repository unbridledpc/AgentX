...
export type SolSettings = {
  ...
  appearancePreset?: "nexus" | "midnight" | "ice";
  accentIntensity?: "soft" | "balanced" | "vivid";
  densityMode?: "compact" | "comfortable" | "airy";
  layout?: LayoutSettings;
};

export const DEFAULT_SOL_SETTINGS: Required<SolSettings> = {
  showInspector: false,
...
function authSession(): { user?: unknown; token?: unknown } | null {
  try {
    const raw = localStorage.getItem("solweb.auth.v1");
    if (!raw) return null;
...
async function handle<T>(res: Response): Promise<T> {
  if (!res.ok) {
    ...
    if (res.status === 401) {
      try {
        localStorage.removeItem("solweb.auth.v1");
        window.dispatchEvent(new Event("solweb:auth-invalid"));
      } catch {
        // ignore
      }
    }
...
export async function getSettings(): Promise<SolSettings> {
  const res = await fetch(`${config.apiBase}/v1/settings`, { headers: authHeaders() });
  return handle(res);
}

export async function saveSettings(settings: SolSettings): Promise<SolSettings> {

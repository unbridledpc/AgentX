...
AgentX is a local-first, supervised AI assistant platform. It combines a Python agent runtime, a FastAPI backend, a React web UI, and an early Tauri desktop client around a policy-aware execution model.
...
- The Python core under `AgentX`.
- The API service under `apps/api`.
- The web UI under `AgentXWeb`.
...
| --- | --- | --- |
| Core runtime | `AgentX/` | Agent, tools, memory, jobs, install/runtime logic, plugins, skills, CLI |
| API service | `apps/api/` | FastAPI service exposing chat, settings, threads, RAG, file, tool, and runtime endpoints |
| Web UI | `AgentXWeb/` | React/Vite browser UI for chat, settings, inspector, code canvas, and runtime controls |
| Desktop UI | `apps/desktop/` | Tauri 2 desktop shell using a React/Vite frontend |
| Installers | `install.sh`, `install-agentx.sh` | Ubuntu/WSL product-style installation and CLI bootstrap |
...
| AgentXWeb dev server | `http://127.0.0.1:5173` |

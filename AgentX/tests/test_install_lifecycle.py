...
from agentx.cli import main as cli_main
from agentx.config import load_config
from agentx.install.deps import install_target_for_config, profile_extra_name, required_dependency_packages
from agentx.install.local_profile import LocalProfileSelection, resolve_local_profile, save_local_profile
from agentx.install.ollama import DEFAULT_OLLAMA_BASE_URL, build_ollama_base_url, detect_wsl_nameserver_ip, wsl_ollama_guidance
from agentx.install.lifecycle import ensure_installation_ready, inspect_runtime, run_doctor, show_paths, start_installation, status_installation, stop_installation, uninstall_installation
from agentx.install.models import InstallProfile, ServiceMode
from agentx.install.store import load_install_config, save_install_config
from agentx.runtime.bootstrap import build_runtime_services_from_config
from agentx.install.wizard import (
    PromptSpec,
...
def _stub_managed_runtime(monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle._ensure_runtime_python_environment", _fake_ensure_runtime_python_environment)
    monkeypatch.setattr("agentx.install.lifecycle.dependency_report", lambda config, python_executable=None: [])
...
def test_profile_defaults() -> None:
    ...
    assert profile_description(InstallProfile.CLI) == "Run AgentX from the terminal only (no web UI)."
    assert profile_description(InstallProfile.STANDARD) == "Recommended: AgentX CLI + API + Web UI."
...
def test_install_config_round_trip(tmp_path: Path) -> None:
    ...
    assert loaded.config_path == (tmp_path / "runtime" / "config" / "agentx.toml").resolve()
...
def test_skipped_profile_uses_os_fallback(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("AGENTX_PROFILE_OS_USER", "agentxtester")
    runtime_root = tmp_path / "runtime"
    save_local_profile(runtime_root, LocalProfileSelection(mode="os-fallback"))
    profile = resolve_local_profile(runtime_root)
    assert profile.mode == "os-fallback"
    assert profile.display_name == "Agentxtester"
    assert profile.profile_id == "agentxtester"


def test_os_fallback_resolution_without_profile_file(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("AGENTX_PROFILE_OS_USER", "fallback-user")
...
def test_validate_install_config_flags_missing_web_dist(tmp_path: Path) -> None:
    app_root = tmp_path / "app"
    (app_root / "AgentX" / "agentx").mkdir(parents=True)
    cfg = build_install_config(
        app_root=app_root,
        runtime_root=tmp_path / "runtime",
        working_dir=tmp_path / "work",
...
def test_prompt_install_config_collects_custom_ollama_endpoint(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "agentx.install.wizard.probe_ollama_endpoint",
...
def test_normalize_setup_path_rejects_backslash_on_linux() -> None:
    ...
    try:
        _normalize_setup_path_input(
            "\\",
            default_value="/home/test/.local/share/agentx",
...
def test_normalize_setup_path_blank_uses_default() -> None:
    ...
    assert path == Path("/home/test/.local/share/agentx").resolve(strict=False)


def test_app_root_sanity_rejects_venv_python_location() -> None:
    error = app_root_sanity_error(Path("/home/agentx/.local/share/agentx/bootstrap/venv/lib/python3.12"))
    assert error is not None
    assert "Invalid AgentX app bundle root detected" in error


def test_fatal_bundle_validation_errors_include_bundle_breakage() -> None:
    errors = (
        "App root does not look like a AgentX bundle: /bad/path",
        "Web UI is enabled but built frontend assets are missing: /bad/path/AgentXWeb/dist/index.html",
    )
...
def test_validate_install_config_warns_for_wsl_mnt_paths(tmp_path: Path, monkeypatch) -> None:
    ...
    (app_root / "AgentXWeb" / "dist" / "index.html").write_text("ok", encoding="utf-8")
    monkeypatch.setattr(
        "agentx.install.wizard.detect_platform",
        lambda: type("Platform", (), {"is_wsl": True, "is_linux": True, "systemd_user_available": False})(),
    )
    cfg = build_install_config(
        app_root=app_root,
        runtime_root=Path("/mnt/e/agentx-runtime"),
        working_dir=Path("/mnt/e/agentx-work"),
        profile=InstallProfile.STANDARD,
        model_provider="stub",
        api_host="127.0.0.1",
        api_port=8420,
...
def test_prompt_install_config_reprompts_on_invalid_runtime_path(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    (app_root / "AgentXWeb" / "dist").mkdir(parents=True)
    (app_root / "AgentXWeb" / "dist" / "index.html").write_text("ok", encoding="utf-8")
    runtime_root = "/home/test/.local/share/agentx"
    work_root = "/home/test/agentx-work"
    responses = iter(
        [
            "standard",
            "\\",
...
def test_prompt_install_config_blank_runtime_path_uses_default(tmp_path: Path, monkeypatch) -> None:
    ...
    (app_root / "AgentXWeb" / "dist").mkdir(parents=True)
    (app_root / "AgentXWeb" / "dist" / "index.html").write_text("ok", encoding="utf-8")
    default_runtime_root = "/home/test/.local/share/agentx"
    work_root = "/home/test/agentx-work"
    responses = iter(
        [
            "standard",
    ...
    monkeypatch.setattr(
        "agentx.install.wizard._runtime_prompt",
        lambda: PromptSpec(
...
def test_prompt_install_config_aborts_on_invalid_app_root(capsys) -> None:
    try:
        prompt_install_config(app_root=Path("/home/agentx/.local/share/agentx/bootstrap/venv/lib/python3.12"))
    except RuntimeError as exc:
        assert "Invalid AgentX bundle root" in str(exc)
    else:
...
def test_ask_renders_prompt_once_in_input_line(capsys, monkeypatch) -> None:
    from agentx.install.wizard import _ask
    ...
    assert captured.out.count("Which address should the AgentX API listen on") == 0
...
def test_wsl_ollama_guidance_includes_nameserver(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("agentx.install.ollama.detect_wsl_nameserver_ip", lambda resolv_conf=Path("/etc/resolv.conf"): "172.28.48.1")
...
def test_render_setup_summary_includes_next_steps(tmp_path: Path) -> None:
    ...
    summary = render_setup_summary(
        ...
        launcher_path="/home/test/.local/bin/agentx",
    )
    assert "AgentX is ready." in summary
    assert "Runtime data:" in summary
    assert "Managed runtime:" in summary
    assert "Provisioning: succeeded" in summary
    assert "Start AgentX:" in summary
    assert "/home/test/.local/bin/agentx" in summary
...
def test_doctor_reports_missing_dependencies(tmp_path: Path, monkeypatch) -> None:
    ...
    ensure_installation_ready(cfg)
    monkeypatch.setattr(
        "agentx.install.lifecycle.dependency_report",
    ...
    monkeypatch.setattr(
        "agentx.install.lifecycle.missing_dependency_messages",
...
def test_doctor_reports_local_profile(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle.dependency_report", lambda config, python_executable=None: [])
    monkeypatch.setattr("agentx.install.lifecycle.missing_dependency_messages", lambda config, python_executable=None: [])
    report = run_doctor(cfg)
...
def test_start_prevents_double_start_and_cleans_stale_pid(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle.dependency_report", lambda config, python_executable=None: [])
    monkeypatch.setattr("agentx.install.lifecycle._process_running", lambda pid: pid in {4242, 4343})
    monkeypatch.setattr("agentx.install.lifecycle._port_open", lambda host, port, timeout_s=0.2: False)
    monkeypatch.setattr("agentx.install.lifecycle._http_health", lambda url, timeout_s=1.0: True)
    ...
    monkeypatch.setattr("agentx.install.lifecycle._spawn_process", _fake_spawn)
...
def test_stop_cleans_stale_pid(tmp_path: Path, monkeypatch) -> None:
    ...
    paths = ensure_installation_ready(cfg)
    paths.api_pid_path.write_text("999999", encoding="utf-8")
    monkeypatch.setattr("agentx.install.lifecycle._process_running", lambda pid: False)
...
def test_uninstall_removes_runtime_bundle_launchers_and_install_config(tmp_path: Path, monkeypatch) -> None:
    ...
    agentx_launcher = launcher_dir / "agentx"
    agentx_launcher = launcher_dir / "agentx"
    marker = f'export AGENTX_BOOTSTRAP_APP_ROOT="{cfg.app_root.resolve(strict=False)}"\n'
    agentx_launcher.write_text(marker, encoding="utf-8")
    agentx_launcher.write_text(marker, encoding="utf-8")
    ...
    monkeypatch.setattr("agentx.install.lifecycle.canonical_user_launcher_path", lambda: agentx_launcher)
    monkeypatch.setattr("agentx.install.lifecycle.compatibility_user_launcher_path", lambda: agentx_launcher)
    monkeypatch.setattr("agentx.install.lifecycle.bootstrap_app_root_record_path", lambda: bootstrap_record)
    monkeypatch.setattr("agentx.install.lifecycle.read_bootstrap_app_root", lambda: cfg.app_root.resolve(strict=False))
    monkeypatch.setattr("agentx.install.lifecycle._process_running", lambda pid: False)
    ...
    assert result["launchers"]["agentx"]["state"] == "removed"
    assert result["launchers"]["agentx"]["state"] == "removed"
...
def test_uninstall_can_keep_app_root(tmp_path: Path, monkeypatch) -> None:
    ...
    install_cfg = tmp_path / "install.json"
    save_install_config(cfg, install_cfg)
    monkeypatch.setattr("agentx.install.lifecycle.canonical_user_launcher_path", lambda: tmp_path / "bin" / "agentx")
    monkeypatch.setattr("agentx.install.lifecycle.compatibility_user_launcher_path", lambda: tmp_path / "bin" / "agentx")
    monkeypatch.setattr("agentx.install.lifecycle.bootstrap_app_root_record_path", lambda: tmp_path / "bootstrap" / "app_root.txt")
    monkeypatch.setattr("agentx.install.lifecycle.read_bootstrap_app_root", lambda: None)
    monkeypatch.setattr("agentx.install.lifecycle._process_running", lambda pid: False)
...
def test_start_failure_reports_log_path_and_log_tail(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle._port_open", lambda host, port, timeout_s=0.2: False)
    state = {"alive": False}
    monkeypatch.setattr("agentx.install.lifecycle._process_running", lambda pid: state["alive"])
...
def test_ensure_installation_ready_provisions_runtime_env(tmp_path: Path, monkeypatch) -> None:
    app_root = tmp_path / "app"
    (app_root / "AgentX").mkdir(parents=True)
    (app_root / "AgentX" / "agentx").mkdir()
    (app_root / "AgentX" / "pyproject.toml").write_text("[project]\nname='agentx'\nversion='0.0.0'\n", encoding="utf-8")
    (app_root / "AgentX" / "plugins").mkdir()
    (app_root / "AgentX" / "skills").mkdir()
    (app_root / "AgentX" / "Server" / "data" / "features").mkdir(parents=True)
    (app_root / "apps" / "api" / "agentx_api").mkdir(parents=True)
    (app_root / "AgentXWeb" / "dist").mkdir(parents=True)
    (app_root / "AgentXWeb" / "dist" / "index.html").write_text("ok", encoding="utf-8")
    ...
    monkeypatch.setattr("agentx.install.lifecycle.venv.EnvBuilder", _FakeEnvBuilder)
    monkeypatch.setattr("agentx.install.lifecycle._run_logged", lambda args, *, cwd, log_path, env=None: seen.append(list(args)))
    monkeypatch.setattr("agentx.install.lifecycle._runtime_environment_needs_install", lambda config, paths: True)
    monkeypatch.setattr("agentx.install.lifecycle._runtime_package_ready", lambda python_path: True)
    monkeypatch.setattr("agentx.install.lifecycle.dependency_report", lambda config, python_executable=None: [])
    paths = ensure_installation_ready(cfg)
...
def test_ensure_installation_ready_fails_when_managed_runtime_missing(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agentx.install.lifecycle.venv.EnvBuilder", _NoopEnvBuilder)
    monkeypatch.setattr("agentx.install.lifecycle._run_logged", lambda *args, **kwargs: None)
    monkeypatch.setattr("agentx.install.lifecycle._runtime_environment_needs_install", lambda config, paths: True)
    monkeypatch.setattr("agentx.install.lifecycle.dependency_report", lambda config, python_executable=None: [])
    try:
        ensure_installation_ready(cfg)
...
def test_doctor_reports_missing_managed_runtime(tmp_path: Path, monkeypatch) -> None:
    ...
    assert report["launcher_path"].endswith("agentx") or report["launcher_path"].endswith("agentx.cmd")
    assert report["runtime_launcher_path"].endswith("agentx") or report["runtime_launcher_path"].endswith("agentx.cmd")
...
def test_runtime_inspect_reports_missing_managed_runtime(tmp_path: Path) -> None:
    ...
    assert report["agentx_module_path"].endswith("agentx\\__init__.py") or report["agentx_module_path"].endswith("agentx/__init__.py")
    assert report["agentx_cli_path"].endswith("agentx\\cli\\__init__.py") or report["agentx_cli_path"].endswith("agentx/cli/__init__.py")
    assert report["launcher_path"].endswith("agentx") or report["launcher_path"].endswith("agentx.cmd")
    assert report["runtime_launcher_path"].endswith("agentx") or report["runtime_launcher_path"].endswith("agentx.cmd")
...
def test_load_config_infers_app_root_from_explicit_app_paths(tmp_path: Path) -> None:
    ...
    app_root = tmp_path / "installed-agentx"
    cfg_path = config_dir / "agentx.toml"
    cfg_path.write_text(
        f"""
[paths]
plugins_dir = "{(app_root / 'AgentX' / 'plugins').as_posix()}"
skills_dir = "{(app_root / 'AgentX' / 'skills').as_posix()}"
features_dir = "{(app_root / 'AgentX' / 'Server' / 'data' / 'features').as_posix()}"
api_dir = "{(app_root / 'apps' / 'api').as_posix()}"
web_dir = "{(app_root / 'AgentXWeb').as_posix()}"
web_dist_dir = "{(app_root / 'AgentXWeb' / 'dist').as_posix()}"
...
def test_doctor_cli_renders_human_friendly_report(tmp_path: Path, capsys, monkeypatch) -> None:
    ...
    assert "agentx" in captured.out.lower() or "agentx" in captured.err.lower()
...
def test_start_cli_renders_failure_guidance_for_missing_runtime(tmp_path: Path, capsys) -> None:
    ...
    (runtime_root / "config" / "agentx.toml").write_text("", encoding="utf-8")
    cfg = build_install_config(
        app_root=app_root,
        runtime_root=runtime_root,
        working_dir=tmp_path / "work",
    ...
    assert "agentx doctor" in captured.out.lower()
...
def test_start_aborts_when_managed_runtime_missing(tmp_path: Path, monkeypatch) -> None:
    ...
    (paths / "agentx.toml").write_text("", encoding="utf-8")
    result = start_installation(cfg, install_config_path=tmp_path / "install.json")
...
def test_internal_serve_api_failure_is_logged(tmp_path: Path, monkeypatch) -> None:
    ...
    save_install_config(cfg, install_cfg)
    monkeypatch.setattr("agentx.cli.serve_api_forever", lambda config: (_ for _ in ()).throw(RuntimeError("boom")))
...
def test_setup_cli_fails_when_provisioning_does_not_create_runtime(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert "AgentX is ready." not in captured.out

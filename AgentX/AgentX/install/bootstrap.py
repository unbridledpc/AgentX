...
def default_bootstrap_root(*, env: dict[str, str] | None = None, home: Path | None = None) -> Path:
    ...
    return (base / "agentx" / "bootstrap").resolve(strict=False)
...
def default_user_launcher_path(*, env: dict[str, str] | None = None, home: Path | None = None, command_name: str = "agentx") -> Path:
...
def ensure_bootstrap_python(
    *,
    ...
    if bootstrap_python is None:
        raise RuntimeError("Bootstrap interpreter path is required. Re-run install-agentx.sh.")
...
def default_runtime_root(*, env: dict[str, str] | None = None, home: Path | None = None) -> Path:
    ...
    xdg_data = (env_map.get("XDG_DATA_HOME") or "").strip()
    base = Path(xdg_data).expanduser() if xdg_data else (home_dir / ".local" / "share")
    return (base / "agentx").resolve(strict=False)


def validate_app_bundle(app_root: Path) -> list[str]:
    required = {
        "AgentX/pyproject.toml": app_root / "AgentX" / "pyproject.toml",
        "apps/api": app_root / "apps" / "api",
        "AgentXWeb": app_root / "AgentXWeb",
    }
    missing: list[str] = []
...
def bootstrap_launcher_content(*, bootstrap_python: Path, app_root: Path) -> str:
    ...
    return (
        "#!/usr/bin/env sh\n"
        "set -eu\n"
        "# Generated by install-agentx.sh. Uses the bootstrap AgentX CLI environment.\n"
        f'BOOTSTRAP_PY="{python_path}"\n'
        'if [ ! -x "$BOOTSTRAP_PY" ]; then\n'
        '  echo "AgentX bootstrap interpreter missing: $BOOTSTRAP_PY" >&2\n'
        '  echo "Re-run install-agentx.sh" >&2\n'
        "  exit 1\n"
        "fi\n"
        f'export AGENTX_BOOTSTRAP_APP_ROOT="{bundle_root}"\n'
        'exec "$BOOTSTRAP_PY" -m agentx "$@"\n'
    )
...
def launcher_validation_error(*, launcher_text: str, bootstrap_python: Path, app_root: Path) -> str | None:
    ...
    export_line = next((line for line in lines if line.startswith('export AGENTX_BOOTSTRAP_APP_ROOT=')), "")
    if export_line != f'export AGENTX_BOOTSTRAP_APP_ROOT="{expected_app_root}"':
    ...
    if exec_line != 'exec "$BOOTSTRAP_PY" -m agentx "$@"':
    ...
    forbidden_exec_patterns = {
        'exec /usr/bin/python3 -m agentx "$@"',
        'exec /usr/bin/python3.12 -m agentx "$@"',
        'exec python3 -m agentx "$@"',
        'exec /usr/bin/env python3 -m agentx "$@"',
    }
...
def read_bootstrap_app_root(*, env: dict[str, str] | None = None, home: Path | None = None, bootstrap_python: Path | None = None) -> Path | None:
    raw = ((env or os.environ).get("AGENTX_BOOTSTRAP_APP_ROOT") or "").strip()
    if raw:
...
def bootstrap_setup_command(*, bootstrap_python: Path, setup_args: list[str] | tuple[str, ...] = ()) -> list[str]:
    python_path = ensure_bootstrap_python(bootstrap_python=bootstrap_python, require_exists=False)
    return [str(python_path), "-m", "agentx", "setup", *[str(arg) for arg in setup_args]]
...
def launcher_targets_bootstrap_python(*, launcher_text: str, bootstrap_python: Path) -> bool:
    ...
    return f'BOOTSTRAP_PY="{expected}"' in launcher_text and 'exec "$BOOTSTRAP_PY" -m agentx "$@"' in launcher_text
...
def shadowed_agentx_warning(*, launcher_path: Path, which_agentx: str | None = None) -> str | None:
    current = which_agentx or shutil.which("agentx")
    if not current:
        return None
    ...
    if resolved == expected:
        return None
    return f"`agentx` currently resolves to {resolved}. The intended bootstrap launcher is {expected}."

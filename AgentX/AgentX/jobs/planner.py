...
from agentx.core.agent import Agent, AgentPolicyError
from agentx.core.runtime_models import Plan, PlanStep
from agentx.core.llm import LlmError, load_ollama_cfg, load_openai_cfg, ollama_generate, openai_chat_completions
from agentx.jobs.models import Job, JobStatus
from agentx.skills.models import SkillRecord
...
class JobPlanner:
    ...
    def _build_prompt(self, *, job: Job, skill: SkillRecord | None, hints: list[Any]) -> str:
        ...
        return (
            "You are AgentX's supervised job planner.\n"
            "Rules:\n"
    ...
    def _generate(self, *, prompt: str, provider: str) -> str:
        if provider == "openai":
            ...
            data = openai_chat_completions(
                cfg=cfg,
                messages=[
                    {"role": "system", "content": "You are AgentX's planner. Return JSON only."},

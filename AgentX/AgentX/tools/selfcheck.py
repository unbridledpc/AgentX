...
from agentx.tools.base import Tool, ToolArgument
...
def _best_effort_version() -> str:
    try:
        ...
        return str(getattr(agentx, "__version__", "") or "") or "<unknown>"
    except Exception:
        return "<unknown>"
...
def run_selfcheck(
    *,
    ...
    run_tool_probe(
        ...
        remediation=[
            "Check [fs].allowed_roots in config/agentx.toml includes this repo path.",
            "Run: python -m agentx tool fs.list --reason SelfCheck --json '{\"path\":\".\",\"max_entries\":10}'",
        ],
    )
    run_tool_probe(
        ...
        remediation=["Ensure AgentX/README.md exists and is under an allowed fs root."],
    )
    run_tool_probe(
        probe_id="fs.grep",
        tool_name="fs.grep",
    ...
    run_tool_probe(
        ...
        remediation=[
            "Ensure AgentX/data is writable and under an allowed fs root.",
            "Check [fs].max_write_bytes in config/agentx.toml.",
        ],
    )
    run_tool_probe(
        probe_id="fs.move",
    ...
    run_tool_probe(
        ...
        remediation=["If cleanup failed, delete AgentX/data/selfcheck/* manually."],
    )
    ...
    run_tool_probe(
        ...
        remediation=["Enable [rag].enabled=true in config/agentx.toml."],
    )
    run_tool_probe(
        probe_id="rag.query",
        tool_name="rag.query",
    ...
    run_tool_probe(
        ...
        remediation=["If blocked, update [web] provider allowlist and [web.search] config in config/agentx.toml."],
    ...
    run_tool_probe(
        ...
        remediation=["If blocked, allowlist wikipedia.org in [web.policy] in config/agentx.toml (deny wins)."],
        warn_on_policy_block=True,
    )
    run_tool_probe(
        probe_id="web.crawl",
    ...
    run_tool_probe(
        ...
        remediation=[
            ...
            "If disabled, enable [tibia.sources] in config/agentx.toml.",
        ],
        warn_on_policy_block=True,
    )
        ...
        try:
            ...
            if not isinstance(out, dict):
                add_result(
                    ...
                    remediation=["Inspect repo.tree tool output shape in agentx/tools/repo.py."],
                )
                return None
            ...
            if status == "WARN":
                ...
                if rate_limited:
                    remediation = [
                        ...
                        "Example: set env var AGENTX_GITHUB_TOKEN and set web.github.token_env = \"AGENTX_GITHUB_TOKEN\".",
                    ]
            if status == "FAIL" and rate_limited:
                remediation = [
                    ...
                    "Verify AgentX/config/agentx.toml has [web.github].token_env set and the env var is visible to the AgentX process.",
                ]
    ...
    run_tool_probe(
        ...
        remediation=["If this fails, inspect agentx/tools/monster.py and its schema."],
    )
    ...
    run_tool_probe(
        ...
        tool_args={"paths": [str(root / "agentx")], "max_files": 3000},
    ...
    run_tool_probe(
        ...
        minimum="full",
        remediation=["Run `python -m pytest -q` in AgentX and inspect failures."],
    )
    ...
    run_tool_probe(
        probe_id="cli.agentx_run_import",
        tool_name="exec.run",
        tool_args={"cmd": "python -c \"import agentx.cli.run; print('OK')\"", "cwd": str(root), "reason": "SelfCheck verify agentx run import"},
        remediation=["Ensure AgentX is importable (run from its root, or fix PYTHONPATH)."],
    )
    run_tool_probe(
        probe_id="cli.agentx_memory_stats",
        tool_name="exec.run",
        tool_args={"cmd": "python -m agentx memory stats --reason SelfCheck", "cwd": str(root), "reason": "SelfCheck memory stats CLI"},
    ...
    if mode == "full":
        ...
        try:
            res = invoker(
                tool_name="exec.run",
                tool_args={
                    "cmd": "python -m agentx memory prune --older-than-days 30 --dry-run --reason SelfCheck",
                    "cwd": str(root),
                    "reason": "SelfCheck memory prune dry-run",
                },
                reason="SelfCheck: cli.agentx_memory_prune",
            )
            t1 = time.perf_counter()
            ...
            if rc != 0:
                ...
                add_result(
                    probe_id="cli.agentx_memory_prune",
                    status="FAIL",
                    duration_ms=(t1 - t0) * 1000.0,
                    detail=f"dry_run_failed rc={rc} stderr={stderr[-200:].strip()}",
                    remediation=["Run manually: python -m agentx memory prune --older-than-days 30 --dry-run --reason SelfCheck"],
                )
            else:
                add_result(
                    probe_id="cli.agentx_memory_prune",
            ...
            add_result(
                ...
                duration_ms=(t1 - t0) * 1000.0,
                detail=str(e) or e.__class__.__name__,
                remediation=["Ensure `python -m agentx memory prune --dry-run` is supported and exec.run is enabled."],
            )
    else:
        add_result(
            probe_id="cli.agentx_memory_prune",
    ...
    run_tool_probe(
        probe_id="cli.agentx_ingest",
        tool_name="exec.run",
        tool_args={
            "cmd": f"python -m agentx ingest --path \"{(root / 'README.md').as_posix()}\" --reason SelfCheck --tags selfcheck --max_files 1",
            "cwd": str(root),
            "reason": "SelfCheck agentx ingest CLI",
        },
    ...
    if mode == "full" and exercise_cli_tool_wrappers:
        ...
        try:
            ...
            res = invoker(
                tool_name="exec.run",
                tool_args={
                    "cmd": f"python -m agentx tool web.ingest_url --reason \"SelfCheck CLI wrapper\" --json \"{payload_escaped}\"",
                    "cwd": str(root),
                    "reason": "SelfCheck agentx tool wrapper",
                },
                reason="SelfCheck: cli.agentx_tool_web_ingest_url",
            )
            t1 = time.perf_counter()
            rc = int(res.get("returncode") or 0) if isinstance(res, dict) else 0
            if rc != 0:
                stderr = str(res.get("stderr") or "") if isinstance(res, dict) else ""
                add_result(
                    probe_id="cli.agentx_tool_web_ingest_url",
                    status="FAIL",
                    duration_ms=(t1 - t0) * 1000.0,
                    detail=f"build_failed returncode={rc} stderr={stderr[-200:].strip()}",
                    remediation=["Run manually: python -m agentx tool web.ingest_url --reason SelfCheck --json '{...}'"],
                )
            else:
                add_result(
                    probe_id="cli.agentx_tool_web_ingest_url",
            ...
            add_result(
                ...
                duration_ms=(t1 - t0) * 1000.0,
                detail=str(e) or e.__class__.__name__,
                remediation=["Ensure `python -m agentx tool ...` works from AgentX root."],
            )
    else:
        add_result(
            probe_id="cli.agentx_tool_web_ingest_url",
    ...
    run_tool_probe(
        ...
        minimum="full",
        remediation=["If this times out, increase [exec].timeout_s in config/agentx.toml or run pytest manually."],
    )
    def npm_build_probe(*, probe_id: str, project_dir: Path | None) -> None:
        ...
        if project_dir is None:
            add_result(
                ...
                status="SKIP",
                duration_ms=0.0,
                detail="not_configured: set [paths].agentxweb_dir / [paths].desktop_dir in config/agentx.toml",
                remediation=[
                    'Example: [paths] agentxweb_dir="F:/openai/AgentXWeb" desktop_dir="F:/openai/apps/desktop"',
                ],
            )
            return
        if not project_dir.exists() or not project_dir.is_dir():
            add_result(
                ...
                remediation=["Fix the configured path in [paths] in config/agentx.toml."],
            )
            return
        ...
        if not npm_ok:
            ...
            add_result(
                ...
                remediation=["Install Node.js/npm and ensure `npm` is on PATH for the shell AgentX uses."],
            )
            return
    ...
    npm_build_probe(probe_id="cli.agentxweb_build", project_dir=getattr(ctx.cfg.paths, "agentxweb_dir", None))
...
class SelfCheckRunTool(Tool):
    ...
    args = (
        ...
        ToolArgument("exercise_cli_tool_wrappers", bool, "Also exercise `python -m agentx tool ...` CLI wrappers (slower)", required=False, default=False),
    )
    ...
    def run(self, ctx, args: dict[str, Any]) -> Any:
        ...
        from agentx.core.unsafe_mode import disable as unsafe_disable
        from agentx.core.unsafe_mode import enable as unsafe_enable
        from agentx.core.unsafe_mode import reset_request_context, set_request_context

...
from agentx.core.memory import Memory, MemoryChunk, MemoryError
from agentx.core.memory_stub import MemoryStub
from agentx.core.llm import (
    LlmError,
    load_ollama_cfg,
    load_openai_cfg,
    ollama_generate,
...
from agentx.core.evidence import EvidenceSource, ExtractedClaim, build_bundle, classify_trust
from agentx.tools.base import ToolExecutionError, ToolValidationError
from agentx.tools.registry import ToolRegistry
from agentx.core.types import VerificationLevel
from agentx.core.runtime_models import AgentResult, ArtifactContext, PendingAction, Plan, PlanStep, RequestAssessment, ToolError, ToolResult
...
class Agent:
    ...
    def __init__(self, *, ctx: AgentXContext, tools: ToolRegistry):
        self.ctx = ctx
        self.tools = tools
        ...
        from agentx.core.memory_policy import MemoryPromotionPolicy
        from agentx.core.orchestrator import RuntimeOrchestrator
    ...
    @classmethod
    def create(cls, *, ctx: AgentXContext, tools: ToolRegistry) -> "Agent":
    ...
    def is_unsafe_enabled(self, thread_id: str | None) -> bool:
        from agentx.core.unsafe_mode import is_unsafe_enabled
    ...
    def memory_prune(self, *, older_than_days: int, reason: str, dry_run: bool = False) -> dict[str, Any]:
        ...
        from agentx.core.unsafe_mode import UNSAFE_BLOCK_MESSAGE, audit_event, is_unsafe_enabled, reset_request_context, set_request_context, summarize_args
    ...
    def _llm_reply(self, *, user_text: str, provider: str, model: str, retrieval_context: str, response_mode: str = "chat") -> str:
        ...
        if prov == "stub" or mdl == "stub":
            return f"AgentX says: {user_text}"
    ...
    def _tool_authority_allowed_status(self, user_text: str) -> tuple[bool, str]:
        ...
        if any(k in low for k in ("fetch ", "open url", "open this url", "open ")) and url:
            ...
            try:
                from agentx.core.web_policy import WebPolicy, is_allowed_url
        ...
        if ("github.com" in low or "repository" in low or "repo" in low) and any(k in low for k in ("xml", "monster", "monsters", "ingest", "learn", "train")):
            ...
            try:
                from agentx.core.web_policy import WebPolicy, is_allowed_url
                from agentx.tools.repo import _parse_repo_url, _github_tree_api_url, _raw_url
    ...
    def _synthesize_with_sources(
        ...
        *,
        ...
        if bundle.verdict == "VERIFIED_PRIMARY" and bundle.claims:
            ...
            prompt = (
                "You are AgentX. Write a concise answer grounded ONLY in the verified facts.\n"
                "STRICT RULES:\n"
    ...
    def _execute_step(self, step: PlanStep, *, prior_results: list[ToolResult]) -> ToolResult:
        ...
        from agentx.core.fs_policy import FsPolicyError, validate_path
        from agentx.core.unsafe_mode import (
    ...
    def _format_tool_results(self, *, plan: Plan, results: list[ToolResult]) -> str:
        ...
        # This keeps AgentXWeb/tool consumers from accidentally ingesting narration.
    ...
    def _post_tool_memory_legacy(self, *, tool, args: dict[str, Any], output: Any) -> None:
        ...
        if tool.name == "repo.ingest":
            ...
            from agentx.core.xml_monster_parser import parse_monster_xml

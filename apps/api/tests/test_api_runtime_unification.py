...
def test_chat_live_route_canvas_artifact_with_filename_writes_file(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(config, "auth_enabled", False)
    monkeypatch.setattr(config, "settings_path", tmp_path / "settings.json")
    threads_dir = tmp_path / "threads"
    threads_dir.mkdir(parents=True, exist_ok=True)
...
def test_chat_live_route_canvas_artifact_without_filename_clarifies(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(config, "auth_enabled", False)
    monkeypatch.setattr(config, "settings_path", tmp_path / "settings.json")
    threads_dir = tmp_path / "threads"
    threads_dir.mkdir(parents=True, exist_ok=True)
...
def test_chat_live_route_write_quoted_text_into_file(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(config, "auth_enabled", False)
    monkeypatch.setattr(config, "settings_path", tmp_path / "settings.json")
    threads_dir = tmp_path / "threads"
    threads_dir.mkdir(parents=True, exist_ok=True)
    ...
    payload = response.json()
    assert "fs.write_text: OK" in payload["content"]
    assert (work_dir / "demo.txt").read_text(encoding="utf-8") == "hello world"


def test_chat_live_route_write_without_content_clarifies(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(config, "auth_enabled", False)
    monkeypatch.setattr(config, "settings_path", tmp_path / "settings.json")
    threads_dir = tmp_path / "threads"
    threads_dir.mkdir(parents=True, exist_ok=True)
...
def test_chat_live_route_artifact_content_overrides_inline_text(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(config, "auth_enabled", False)
    monkeypatch.setattr(config, "settings_path", tmp_path / "settings.json")
    threads_dir = tmp_path / "threads"
    threads_dir.mkdir(parents=True, exist_ok=True)
    ...
    response = client.post(
        "/v1/chat",
        json={
            "message": 'save "inline text" as demo.py',
            "thread_id": None,
            "active_artifact": {
                "source": "canvas",
                "type": "code",
